...
### Current Power-2 hybrid rotation layer

`Power2HybridRotation16Bit` is the current normal-rotation candidate. It combines fixed16 Quaternion-4 Power-2 Numerel with Gray-preserving zero-bone RLE. A bone is RLE-eligible only when all four scalar differences would use Numerel's zero Golomb symbol. The RLE form omits those four one-bit Golomb-zero markers but still transmits all four Gray correction bits, so a skipped zero bone follows the same receiver-state transition as four ordinary zero Numerel scalars. RLE metadata uses unsigned Exp-Golomb run count, gap, and run-length values, and the encoder chooses the RLE form only when the exact bit count is smaller than the ordinary stream.

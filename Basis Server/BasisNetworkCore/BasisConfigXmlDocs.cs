...
namespace Basis.Network.Core
{
    ...
    public static class BasisConfigXmlDocs
    {
        ...
        private static void RegisterServerConfig()
        {
            ...
            t.Fields.Add(new FieldDoc("BSRMaxDegreeOfParallelism", " Worker policy for the Server Reduction System's parallel phases (send loop, message processing, distance sweep). int; 0 = throughput auto-tune and recommended. Auto mode starts from a population-based estimate, then probes worker counts and directly measures BSR pair sends/second; it keeps the count that produces the most updates, preferring fewer LiteNetLib socket bytes per delivered BSR update when throughput is statistically tied, then lower slicing/shedding and fewer workers, and periodically revalidates as machine/load conditions change. The search is bounded by the machine core count and resets after material population changes. A positive value disables auto-tuning and pins that many workers (clamped to the core count). Watch '[BSR AutoTune]' and '[CPU/POP]' for the selected worker count and measured update rate. "));
